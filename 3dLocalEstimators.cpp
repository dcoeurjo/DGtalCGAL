                        ...
                        const std::string & nameVolFile = "" )
{
  ...
  try
  {
    ...
    std::cout << "# Mean Curvature estimation from Monge polynomial fitting" << std::endl;
    ...
    typename ReporterH::SurfelConstIterator aabegin = surf.container().begin();
    typename ReporterH::SurfelConstIterator aaend = surf.container().end();

    std::vector<double> MongeMeanCurvatures =
      estimateQuantity( reporterH, aabegin, aaend );
    ...
    std::vector<double> MongeGaussianCurvatures =
      estimateQuantity( reporterK, aaabegin, aaaend );
    double TMongeGaussianCurv = c.stopClock();
    ...
    std::cout << "# Shape = " << name << std::endl
              ...
              << "# Time-MongeFittingMeanCurvature = " << TMongeMeanCurv << std::endl
              << "# Time-MongeFittingGaussianCurvature = " << TMongeGaussianCurv << std::endl
              << "# id x y z TrueMeanCurvature TrueGaussianCurvature"
              << " IIMeanCurvature IIGaussianCurvature"
              << " MongeFittingMeanCurvature MongeFittingGaussianCurvature"
              << std::endl;

    VisitorRange range6( new Visitor( surf, *surf.begin() ) );
    abegin = range6.begin();
    aend = range6.end();
    ...
    for ( int i = 0; abegin != aend; ++abegin, ++i )
    {
      ...
      //We use II to fix the MongeMeanCurvature sign
      if (IIMeanCurvatures[ i ]*  MongeMeanCurvatures[ i ]<0.0)
        MongeMeanCurvatures[ i ] *= -1.0;
      ...
      std::cout << i << std::setprecision( 15 )
                ...
                << " " << MongeMeanCurvatures[ i ]
                << " " << MongeGaussianCurvatures[ i ]
                << std::endl;
